/**
 * Resident Controller
 * Extracted logic from residentRoutes.js
 */
 ...
 */
export const getFavoriteVisitors = async (req, res) => {
    const userId = req.user.id;
 ...
 */
export const addFavoriteVisitor = async (req, res) => {
    const userId = req.user.id;
    ...
    const result = await dbManager.query(
     ...
     RETURNING *`,
        [userId, targetVisitorId, nickname || visitor_name || 'My Favorite', relationship || 'Guest', notes || '']
    );
 ...
 */
export const removeFavoriteVisitor = async (req, res) => {
    const userId = req.user.id;
    const { id } = req.params;
