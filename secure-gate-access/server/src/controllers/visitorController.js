...
const createVisitor = async (req, res) => {
  ...
  try { await req.audit?.('invite.create', 'visitor', String(visitor.id), { inviteCode }); } catch {}
...
const createPass = async (req, res) => {
  try {
    ...
    try {
      ...
      if (!visitor) { await client.query('ROLLBACK'); return respond(res, { success: false, error: 'Visitor not found', code: 404 }); }
      ...
      if (exists.rowCount > 0) { await client.query('ROLLBACK'); return respond(res, { success: false, error: 'Active pass already exists', code: 409 }); }
      ...
      try { qrCodeData = await qrcode.toDataURL(passId); } catch {
        ...
        return respond(res, { success: false, error: 'Failed to generate QR', code: 500 });
      }
      ...
      await auditLog(req, { userId: req.user?.id, action: 'visitor.pass.create', entityType: 'pass', entityId: String(passRes.rows[0].id), outcome: 'success', message: 'Pass created for visitor', metadata: { visitorId: Number(visitorId), expiresAt: expiresAt.toISOString() } });
    ...
    await auditLog(req, { userId: req.user?.id, action: 'visitor.pass.create', entityType: 'pass', outcome: 'fail', message: 'Failed to create pass', metadata: { error: String(error?.message) } });
...
const completeInvite = async (req, res) => {
  try {
    ...
    if (visitor && visitor.date_of_visit) {
      ...
      if (visitDate < today) return respond(res, { success: false, error: 'Invitation expired', code: 410 });
    }
    if (!visitor) {
  ...
  try { await req.audit?.('otp.issue', 'visitor', String(visitor.id), { ttl: OTP_TTL_MINUTES }); } catch {}
...
const verifyOtp = async (req, res) => {
  try {
    ...
    // Success: clear OTP to prevent reuse and mark confirmed; reset resend counters
    await pool.query(
      `UPDATE visitors
         SET otp_hash = NULL,
             otp_expires_at = NULL,
  ...
  try { await req.audit?.('otp.verify', 'visitor', String(v.id), { ok: true }); } catch {}
...
// Resend OTP with cooldown and daily cap (persisted)
const resendOtp = async (req, res) => {
  try {
    ...
    const r = await pool.query(
      `SELECT id, email, phone, status, otp_resend_count, otp_last_resend
    ...
    // Persistent throttle checks: 60s cooldown and max 5 per day
    const lastMs = v.otp_last_resend ? new Date(v.otp_last_resend).getTime() : 0;
    ...
    // Derive day scope from last resend; if last resend is a different day, reset count on update
    const isSameDay = v.otp_last_resend && (new Date(v.otp_last_resend).toISOString().slice(0,10) === dayKey);
    const dayCount = isSameDay ? (v.otp_resend_count || 0) : 0;
    if (dayCount >= 5) {
      metrics.otp_resend_rate_limited = (metrics.otp_resend_rate_limited||0) + 1; await maybeAlert('otp_resend_rate_limited');
    ...
    try {
      ...
      await client.query(
        `UPDATE visitors
            SET otp_hash=$1,
                ...
                otp_attempts=0,
                status = 'OTP_SENT',
                otp_resend_count = CASE WHEN otp_last_resend IS NULL OR CAST(otp_last_resend AS DATE) <> CAST(NOW() AS DATE)
                                         THEN 1 ELSE (COALESCE(otp_resend_count,0) + 1) END,
                otp_last_resend = NOW()
          WHERE id=$3`,
      ...
      await client.query(
        `INSERT INTO otp_resend_log (visitor_id, channel, success, metadata)
           VALUES ($1, $2, NULL, $3)`,
        [v.id, 'init', JSON.stringify({ reason: 'user_requested', ip: req.ip || null, ua: req.headers['user-agent'] || null })]
      );
      await client.query('COMMIT');
    } catch (e) {
      try { await client.query('ROLLBACK'); } catch {}
  ...
  // Deliver via available channels, respecting resident preferences
  let notify_email = true, notify_sms = false;
  let residentEmail = null;
  if (v && v.created_by) {
    residentEmail = v.created_by;
    const prefRes = await pool.query('SELECT notify_email, notify_sms FROM users WHERE email = $1', [residentEmail]);
    if (prefRes.rowCount > 0) {
      notify_email = prefRes.rows[0].notify_email;
      notify_sms = prefRes.rows[0].notify_sms;
  ...
  const deliveries = [];
  if (v.email && notify_email) deliveries.push(sendEmailOtp(v.email, otp));
  if (v.phone && notify_sms) deliveries.push(sendSmsOtp(v.phone, otp));
  const results = await Promise.allSettled(deliveries);
    const delivered = results.some(r => r.status === 'fulfilled' && r.value === true);
    ...
    // Persist delivery outcome into log (best-effort)
    try {
      await pool.query(
        `INSERT INTO otp_resend_log (visitor_id, channel, success, metadata)
           VALUES ($1, $2, $3, $4)`,
        [v.id, delivered ? 'multi' : 'none', delivered, JSON.stringify({ channels: { email: !!v.email, phone: !!v.phone } })]
      );
    } catch {}
...
const checkInVisitor = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (r.rowCount === 0) { await client.query('ROLLBACK'); return respond(res, { success: false, error: 'Visitor not
