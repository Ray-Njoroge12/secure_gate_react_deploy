...
BEGIN
  ...
  IF notifications_exists THEN
    ...
    -- Create indexes if notifications table exists
  ...
  ELSE
    RAISE NOTICE 'Skipping notifications table modifications - table does not exist';
  END IF;
END $$;

-- Comments for notifications columns (only apply if table exists)
DO $$
DECLARE
  notifications_exists BOOLEAN;
BEGIN
  ...
  IF notifications_exists THEN
    ...
    COMMENT ON COLUMN notifications.delivery_provider IS 'Provider: mailgun, africas_talking, smtp';
...
BEGIN
    ...
    RAISE NOTICE '🔔 Webhook endpoints ready for: Mailgun, Africa''s Talking';
END $$;
