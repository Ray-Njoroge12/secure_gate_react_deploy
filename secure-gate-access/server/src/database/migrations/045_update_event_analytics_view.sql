-- Update event_analytics view to include estate_location_id and fix scoping
DROP VIEW IF EXISTS event_analytics CASCADE;

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'events')
       AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'event_visitors') THEN
        EXECUTE $view$
