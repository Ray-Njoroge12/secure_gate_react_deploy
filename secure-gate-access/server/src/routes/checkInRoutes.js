  ...
  const result = await dbManager.query(
    `UPDATE visitors 
     ...
     WHERE id = $4
       AND estate_id = $5
     RETURNING *`,
    [PASS_STATUS.CHECKED_IN, guardId, notes, visitorData.id, estateId]
  );
  ...
  const result = await dbManager.query(
    `UPDATE visitors 
     ...
     WHERE id = $5
       AND estate_id = $6
     RETURNING *`,
    [PASS_STATUS.CHECKED_IN, guardId, notes, vehicle_plate, visitorId, estateId]
  );
