...
async function main() {
  try {
    ...
    const sql = fs.readFileSync('migrations/20250915_performance_indexes.sql', 'utf8');
    
    // Split by CREATE INDEX statements
    const indexStatements = sql.split(/CREATE INDEX/i).filter(s => s.trim());
    
    console.log(`📝 Found ${indexStatements.length - 1} index statements to apply`);
    ...
    console.log('\n🎉 Database Index Migration Complete!');
    console.log(`✅ Successfully applied ${successCount} performance indexes`);
