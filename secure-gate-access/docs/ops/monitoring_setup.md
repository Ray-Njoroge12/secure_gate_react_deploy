...
| Notification Queue Size | < 100 | > 1000 |
| Notification Processing Time | < 30s | > 120s |
...
const businessMetrics = {
  ...
  // Intelligent Notification Metrics
  notification_delivery_rate: 'sent_notifications / total_notifications',
  notification_engagement_rate: 'clicked_notifications / delivered_notifications',
  avg_notification_processing_time: 'AVG(sent_at - created_at)',
  notification_queue_health: 'COUNT(*) FROM notification_queue WHERE status = "pending"',
  user_preference_adoption_rate: 'users_with_preferences / total_users',
  quiet_hours_effectiveness: 'notifications_read_in_optimal_hours / total_notifications_read'
};
```
