...
import { getAllResidents, updateResident, deleteResident } from "../../services/adminService";
...
const ResidentCard = ({ resident, onEdit, onDeactivate, onEmail }) => (
  <Card className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 hover:shadow-md dark:hover:shadow-slate-700 transition-shadow">
          ...
          <div className="flex items-center gap-2 mb-2">
            <div className="w-10 h-10 bg-brand-100 rounded-full flex items-center justify-center">
              <span className="text-brand-600 font-semibold">
                {(resident.name || resident.username || 'U').charAt(0).toUpperCase()}
              </span>
            </div>
            <div>
...
const EditResidentModal = ({ resident, isOpen, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (resident) {
      setFormData({
        name: resident.name || resident.username || '',
...
export default function ManageResidents() {
  ...
  const searchFields = ['name', 'username', 'email', 'phone', 'area', 'house_number'];
  const filterFields = [
  ...
  const handleDeactivate = async (resident) => {
    const confirmed = await confirm?.({
      ...
      message: `Are you sure you want to deactivate ${resident.name || resident.username}? They will no longer be able to log in.`,
      variant: 'danger',
  ...
  const handleExport = () => {
    const csv = [
      ['ID', 'Name', 'Email', 'Phone', 'Area', 'House Number', 'Status'],
      ...users.map(u => [u.id, u.name || u.username, u.email, u.phone, u.area, u.house_number, u.status])
  ...
  return (
    <AppShell role={role} title="Manage Residents">
          ...
          <Card.Content className="p-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  type="search"
                  placeholder="Search residents by name, email, phone, or area..."
                  value={searchTerm}
